...
const formatPhoneInput = () => {
  ...
  if (number.length === 0) {
    ...
    reporter_phone_number.value = `(${number.slice(0, 3)}) ${number.slice(3)}`;
  } 
  
  else {
    reporter_phone_number.value = `(${number.slice(0, 3)}) ${number.slice(3, 6)}-${number.slice(6, 10)}`;
  }
}
    ...
    <div class="pt-5 grid grid-cols-12 content-center place-items-center">
      ...
      <div class="col-span-12 grid grid-cols-12">
        <div class="col-span-4">
          ...
          <label for="reporter_legal_name" class="mr-2 block content-center pt-2">Your Name</label>
          <input type="text" name="reporter_legal_name" v-model="reporter_legal_name" id="reporter_legal_name" placeholder="Jane Doe" class="border rounded-lg p-2">   
        </div>
        
        <div class="col-span-4 mx-4">
          <label for="name" class="mr-2 block content-center">Last Location Seen</label>
          <input type="text" name="last_location_seen" v-model="last_location_seen" id="last_location_seen" placeholder="Moose Jaw, SK" class="border rounded-lg p-2">

          <label for="name" class="mr-2 block content-center pt-2">Last Date & Time Seen</label>
          <input type="datetime-local" name="last_date_time_seen" v-model="last_date_time_seen" id="last_date_time_seen" class="border rounded-lg p-2">
          
          <label for="reporter_phone_number" class="mr-2 block content-center pt-2">Your Phone Number</label>
          <input type="text" name="reporter_phone_number" v-model="reporter_phone_number" id="reporter_phone_number" @input="formatPhoneInput" placeholder="(123) 456-7890" class="border rounded-lg p-2">   
        </div>
        ...
        <div class="col-span-4 justify-self-center">
          ...
          <!-- Placeholder Image -->
          <div v-if="image_uri">
            <img :src="image_uri" class="block justify-center overflow-hidden w-24 h-24" alt="Image" height="90" width="90" id="image" name="image">
          </div>
          ...
          <input type="file" id="image_input" name="image" accept="image/png, image/jpg, image/jpeg" class="hidden" @change="handleImageChange" />
          <label for="image_input" class="cursor-pointer underline mt-1 font-bold text-sm">Choose Image</label>
        </div>
      </div>

      <div class="col-span-12 py-2">
        <label for="additonalInfo" class="mr-2 block content-center">Additional Info</label>
        <textarea name="additional_info" v-model="additional_info" id="additional_info" cols="78" rows="5" wrap="hard" maxlength="200" class="border rounded-lg p-2 resize-none" placeholder="Add extra information about this person here. (optional)"></textarea>
      </div>
    </div>
